...
static const unsigned char DBTIXML_UTF_LEAD_0 = 0xefU;
static const unsigned char DBTIXML_UTF_LEAD_1 = 0xbbU;
static const unsigned char DBTIXML_UTF_LEAD_2 = 0xbfU;
...
const char *XMLUtil::ReadBOM(const char *p, bool *bom)
{
    *bom = false;
    ...
    // Check for BOM:
    if (*(pu + 0) == DBTIXML_UTF_LEAD_0
            && *(pu + 1) == DBTIXML_UTF_LEAD_1
            && *(pu + 2) == DBTIXML_UTF_LEAD_2)
    {
        *bom = true;
        p += 3;
    }
...
{
    DBTIXML_SNPRINTF(buffer, bufferSize, "%d", v);
}


void XMLUtil::ToStr(unsigned v, char *buffer, int bufferSize)
{
    DBTIXML_SNPRINTF(buffer, bufferSize, "%u", v);
}


void XMLUtil::ToStr(bool v, char *buffer, int bufferSize)
{
    DBTIXML_SNPRINTF(buffer, bufferSize, "%d", v ? 1 : 0);
}


void XMLUtil::ToStr(float v, char *buffer, int bufferSize)
{
    DBTIXML_SNPRINTF(buffer, bufferSize, "%g", v);
}
...
{
    if (DBTIXML_SSCANF(str, "%u", value) == 1)
    {
        return true;
    }
...
{
    if (DBTIXML_SSCANF(str, "%f", value) == 1)
    {
        return true;
    }
...
{
    if (DBTIXML_SSCANF(str, "%lf", value) == 1)
    {
        return true;
    }
...
void XMLDocument::PrintError() const
{
    if (_errorID)
    {
        ...
        if (_errorStr1)
        {
            DBTIXML_SNPRINTF(buf1, LEN, "%s", _errorStr1);
        }
        
        if (_errorStr2)
        {
            DBTIXML_SNPRINTF(buf2, LEN, "%s", _errorStr2);
        }
...
{
    static const unsigned char bom[] = { DBTIXML_UTF_LEAD_0, DBTIXML_UTF_LEAD_1, DBTIXML_UTF_LEAD_2, 0 };
